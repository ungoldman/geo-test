...
function runTest(type){
  $('.result').empty().removeClass('pass fail');
  switch (type) {
    case 'existence':
      existenceTest();
      break;
...
function getCurrentPositionTest(){
  ...
  success = function(position){
    $el.find('.result').text(position).addClass('pass');
  ...
  error = function(error){
    ...
    $el.find('.result').text(text).addClass('fail');
    log('error', error);
  }
...
function watchPositionTest(){
  ...
  success = function(position){
    ...
    $el.find('.result').text(d + ' -- ' + position).addClass('pass');
    log('position', position);
  };
